...
export default function EditInWarranty({ onClose, currentConfig, onSaveSuccess }) {
    const [serviceCharge, setServiceCharge] = useState(currentConfig?.serviceCharge || 0);
    const [replacementCharge, setReplacementCharge] = useState(currentConfig?.replacementCharge || 0);
    const [tncFile, setTncFile] = useState(null);
    const [existingTncUrl, setExistingTncUrl] = useState(currentConfig?.termsAndConditionsUrl || '');
    ...
    const handleSave = async () => {
        ...
        formData.append('replacementCharge', replacementCharge);
        if (tncFile) {
            formData.append('termsAndConditions', tncFile);
        ...
        try {
            ...
            const res = await axios.put(`${import.meta.env.VITE_API_URL}/api/billing-config/in-warranty`, formData, {
                headers: {
