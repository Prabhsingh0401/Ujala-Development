...
export default function InWarrantyOutput({ onEdit, onConfigLoaded, refresh }) {
    ...
    // Dummy data for now
    const serviceCharge = config?.serviceCharge || 0;
    const replacementCharge = config?.replacementCharge || 0;
    const tncUrl = config?.termsAndConditionsUrl ? `${import.meta.env.VITE_API_URL}/${config.termsAndConditionsUrl}` : null;

    return (
        <div className="bg-white p-6 rounded-lg shadow-md w-full mx-auto">
            <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-semibold">In Warranty Billing Details</h3>
                <button
                    onClick={onEdit}
